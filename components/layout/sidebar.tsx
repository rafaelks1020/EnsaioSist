...
export function Sidebar({ children }: SidebarProps) {
  ...
  // Melhorar detecção da área atual
  const isAcessoArea = pathname.startsWith('/acesso');
  const isAdminArea = pathname.startsWith('/admin');
  const isAppArea = pathname.startsWith('/app');
  
  // Filtrar itens de navegação baseado na área e permissões
  let navItems: NavItem[] = [];
  
  if (isAcessoArea) {
    // No acesso area, always show its navigation to avoid empty menu for admins/users
    navItems = acessoNavigation;
  } else {
...
function SidebarContent({ navItems, pathname, session, onSignOut, isAcessoArea }: SidebarContentProps) {
  return (
    <div className="flex flex-col flex-grow bg-gradient-to-br from-purple-900 via-indigo-900 to-pink-900 relative">
      {/* Background Musical */}
        ...
        <div className="mt-5 flex-grow flex flex-col">
          <nav className="flex-1 px-2 space-y-1">
            {navItems.map((item) => {
              ...
              // Melhorar detecção de link ativo para incluir subpáginas
              ...
              return (
                <Link
                  ...
                  } group flex items-center px-3 py-3 text-sm font-medium rounded-md transition-all touch-manipulation`}
                >
                  <Icon
                    className={`${
            ...
            <div className="border-t border-white/20 pt-4">
              <div className="px-2 space-y-1">
                ...
                {!isAcessoArea && session.user.role !== Role.ADOLESCENTE && (
                  <Link
                    href="/acesso/hinos"
                    className="text-gray-200 hover:bg-white/10 hover:text-white group flex items-center px-3 py-3 text-sm font-medium rounded-md transition-all touch-manipulation"
                  >
                ...
                <Button
                  className="w-full justify-start text-gray-200 hover:bg-white/10 hover:text-white bg-transparent border-none px-3 py-3 h-auto transition-all touch-manipulation"
                  onClick={onSignOut}
                >
